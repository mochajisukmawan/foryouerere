...
header("Content-Disposition: attachment; filename=Export_excel_peserta.xls");  //File name extension was wrong
header("Expires: 0");
...
$q=$this->db->query("SELECT distinct
	...
	a.`pajak`,
	a.`take_home_pay`
FROM payroll a JOIN view_peserta_magang b ON (a.`nomor_register`=b.`nomor_register` AND a.`verify`=1 AND a.`tahun`=? AND a.`bulan`=?) ORDER BY a.`unit_kerja_id` ASC;",array($tahun,$bulan))->result();
?>
<style type="text/css">
