...
$q=$this->db->query("SELECT distinct
	...
	a.`pajak`,
	a.`take_home_pay`
FROM payroll a JOIN peserta_magang b ON (a.`nomor_register`=b.`nomor_register` )  WHERE a.`verify`=1 AND a.`tahun`=? AND a.`bulan`=? AND a.nomor_register IN (SELECT pm.`nomor_register` FROM peserta_magang pm JOIN pembimbing_magang_peserta pmp on pm.nomor_register = pmp.nomor_register WHERE pmp.aktif='3' AND pm.`selesai_level`='Tahap-Dasar' AND pm.`nomor_register` NOT IN (
                SELECT nomor_register FROM view_peserta_magang v) AND YEAR(pmp.`tanggal_selesai`)='$tahun' AND MONTH(pmp.`tanggal_selesai`)='$bulan') ORDER BY a.`unit_kerja_id` ASC;",array($tahun,$bulan))->result();
?>
<style type="text/css">
